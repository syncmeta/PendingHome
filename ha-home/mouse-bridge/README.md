...
三只无线鼠标当灯的开关，接收器都插在这台实体机上。

| 鼠标 | 设备标识 | 控哪盏 | 滚轮能调 |
|---|---|---|---|
| MI Wireless Mouse（1-1.5 口）| `2717:003b@1-1.5` | 350 吸顶灯 `light.mismartledceilinglight350_2545_...` | 亮度 + 色温（中键切换）|
| MI Wireless Mouse（1-1.6 口）| `2717:003b@1-1.6` | 餐厅灯 `light.yeelink_cn_56292508_mono1_s_2_light` | 只有亮度（这灯不支持色温）|
| MiMouse 2 | `2717:501f` | 450 主卧吸顶灯 `light.mismartledceilinglight450_e290_...` | 亮度 + 色温（中键切换）|
...
**① 两只 MI Wireless Mouse 的接收器在系统里完全一样，只能靠 USB 插口区分。**
同样的 `2717:003b`、同样的名字，USB 描述符里连序列号都没有。所以它们的设备标识
必须写成 `2717:003b@1-1.5` 这种带插口的形式（`--list` 检测到同款有多个时会直接
给出这个写法）。**代价：标识跟的是插口不是鼠标，接收器换个口插绑定就跟着变。**
MiMouse 2 的 `2717:501f` 是独一份，所以用不带插口的写法，换口也认。

**② 无线鼠标睡着之后，第一下点击收不到。**
实测确认过：控灯服务和另开的原始事件记录器**两条独立的线同时为空**，那一下
根本没到过这台机器 —— 被鼠标用来唤醒自己的无线电了。这是「电池 + 维持链路」
这类设备的固有行为，主机端无解（接收器这边已经查过不省电：`power/control` 是
`on`，`runtime_suspended_time` 是 0）。要彻底解决只能换不睡觉的东西：有线 USB
按键，或者 Zigbee 这种「按下即发」的无线开关。

**③ 吸顶灯走 HomeKit 本地接入，但它自报的色温上限是假的。**
两盏吸顶灯（350 和 450）在 HA 里各有两份实体：米家云那份和 HomeKit 本地那份。
绑定用本地这份 —— 不绕小米服务器、断网也能控。怎么确认两份是同一盏灯：
HomeKit 那份的 `serial_number` 就是米家那份的 did（350 是 `476282703`，
450 是 `581700272`），不是靠名字猜的。

代价是 HomeKit 把色温上限报成 **20000K**，而灯实际只到 6100K（米家那份报的才对）。
不夹住的话滚轮会一路滚到两万，灯早就不动了，用起来像坏了。所以 `config.json` 里
给这两盏都写了 `"color_temp_kelvin_range": [2600, 6100]` —— 这个字段是可选的，
只有实体自报的区间不可信时才需要写。
