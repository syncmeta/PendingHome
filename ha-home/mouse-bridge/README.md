...
| 鼠标 | 设备标识 | 接法 | 控哪盏 | 滚轮能调 |
|---|---|---|---|---|
| MI Wireless Mouse | `2717:003b` | 2.4G USB 接收器 ✅ | 吸顶灯 `light.mismartledceilinglight350_2545_mi_smart_led_ceiling_light_sw` | 亮度 + 色温（中键切换）|
| MiMouse 2 | `2717:501f` | **蓝牙** ⚠️ 见下 | 餐厅灯 `light.yeelink_cn_56292508_mono1_s_2_light` | 只有亮度（这灯不支持色温）|
...
**① MiMouse 2 是蓝牙鼠标，不是 2.4G。** 在 Mac 上两只鼠标都能读到，是因为 macOS
把蓝牙 HID 也当普通 HID 设备暴露给 IOHIDManager，看不出区别。到了 Linux 才发现：
`2717:501f` 没有配套接收器可插，要走蓝牙配对，而这台奔腾主板**没有蓝牙**
（内核蓝牙协议栈在，`/sys/class/bluetooth/` 是空的 —— 有软件没硬件）。
要让它工作，二选一：插一个 USB 蓝牙适配器再配对，或者换一只带 2.4G 接收器的鼠标。

**② 吸顶灯走 HomeKit 本地接入，但它自报的色温上限是假的。**
同一盏灯 HA 里有两份实体：米家云那份（`light.yeelink_cn_476282703_ceiling23_s_2_light`）
和 HomeKit 本地那份。绑定用本地这份 —— 不绕小米服务器、断网也能控。
代价是 HomeKit 把色温上限报成 **20000K**，而灯实际只到 6100K（米家那份报的才对，
两份实体的设备序列号都是 `476282703`，是同一盏灯）。不夹住的话滚轮会一路滚到两万，
灯早就不动了，用起来像坏了。所以 `config.json` 里给它写了
`"color_temp_kelvin_range": [2600, 6100]` —— 这个字段是可选的，只有实体自报的区间
不可信时才需要写。
