...
# ---- USB_VBUS:y130.6 走廊 ----
p1 = [(44.36,127.1),(52.0,130.6),(73.6,130.6)]
p2 = [(73.6,130.6),(73.6,138.53),(73.6,140.85)]
p3 = [(73.6,140.85),(78.4,140.85)]
p4 = [(78.4,140.85),(78.4,138.53)]
okv = check(p1, 0.25, B, "USB_VBUS", verbose="VBUS-1") and \
      chkvia(73.6,130.6,"USB_VBUS",verbose="VBUS") and \
...
# ---- CC2:东移起步 ----
p_s = [(68.85,136.6),(69.6,136.05),(70.0,135.85)]
p_e = [(76.9,135.9),(77.75,136.7),(77.75,138.4)]
if check(p_s, 0.125, F, "CC2", verbose="CC2-s") and chkvia(70.0,135.85,"CC2",verbose="CC2") and \
   check([(70.0,135.85),(76.9,135.9)], 0.125, B, "CC2", verbose="CC2-b") and \
   chkvia(76.9,135.9,"CC2",verbose="CC2v2") and \
   check(p_e, 0.125, F, "CC2", verbose="CC2-e", margin=0.18):
    trk(p_s, "CC2", 0.25)
    via(70.0,135.85,"CC2")
    trk([(70.0,135.85),(76.9,135.9)], "CC2", 0.25, layer=B)
    via(76.9,135.9,"CC2")
    trk(p_e, "CC2", 0.25)
else:
    WARN.append("CC2")
...
# ---- LED6_K:过孔东移 ----
if chkvia(16.6,83.0,"LED6_K",verbose="LED6K") and chkvia(17.31,81.9,"LED6_K",verbose="LED6K2") and \
   check([(16.6,83.0),(17.31,81.9)], 0.15, B, "LED6_K", verbose="LED6K-B") and \
   check([(17.31,81.9),(17.31,81.0)], 0.15, F, "LED6_K", verbose="LED6K-F"):
    trk([(16.05,83.56),(16.6,83.1),(16.6,83.0)], "LED6_K", 0.3)
    via(16.6,83.0,"LED6_K")
    trk([(16.6,83.0),(17.31,81.9)], "LED6_K", 0.3, layer=B)
    via(17.31,81.9,"LED6_K")
    trk([(17.31,81.9),(17.31,81.0)], "LED6_K", 0.3)
else:
    WARN.append("LED6_K")

# ---- CH4 CW_GR:利用东端既有过孔 (71,40.8),向西大绕 ----
# 找 F 跨越段(与 WW 对角线相交)拿到南端点 Q
diag = (65.1,42.6,70.0,45.6)
crosser = None
for t in live():
    if is_via(t) or t.GetNetname() != "CH4_CW_GR" or t.GetLayer() != F:
        continue
    if ssd(diag, ends(t)) < 0.85:
        crosser = t
        break
if crosser:
    x1, y1, x2, y2 = ends(crosser)
    Q = (x2, y2) if y2 > y1 else (x1, y1)
    kill(crosser)
    del_via_at([(64.6,40.8)])
    del_exact("CH4_CW_GR", [(64.6,40.8,71.0,40.8)])
    p_b = [(71.0,40.8),(66.3,45.0),(64.0,52.0),(64.0,66.0),Q]
    if check(p_b, 0.15, B, "CH4_CW_GR", verbose="CH4g-B") and chkvia(*Q, "CH4_CW_GR", verbose="CH4g"):
        trk(p_b, "CH4_CW_GR", 0.3, layer=B)
        via(*Q, "CH4_CW_GR")
        print(f"✓ CH4 CW_GR 绕行 → Q=({Q[0]:.2f},{Q[1]:.2f})")
    else:
        WARN.append("CH4 CW_GR")
        trk([(x1,y1),(x2,y2)], "CH4_CW_GR", 0.25)
        via(64.6,40.8,"CH4_CW_GR")
        trk([(64.6,40.8),(71.0,40.8)], "CH4_CW_GR", 0.25, layer=B)
else:
    print("⚠️ CH4 跨越段未找到(可能已处理)")
