...
# ---- USB_VBUS:终点收到 x73.3(EN 斜线),再斜入 A4B9 ----
p1 = [(44.36,127.1),(52.0,130.6),(73.3,130.6)]
p2 = [(73.3,130.6),(73.3,137.9),(73.6,138.53),(73.6,140.85)]
...
okv = check(p1, 0.25, B, "USB_VBUS", verbose="VBUS-1") and \
      ...
      check(p2, 0.15, F, "USB_VBUS", verbose="VBUS-2", margin=0.18) and \
      ...
      check(p4, 0.15, F, "USB_VBUS", verbose="VBUS-4", margin=0.18)
if okv:
...
# ---- CC2:R10 竖放 (69.6,136.5),pad1 朝北直出 ----
for fp in board.GetFootprints():
...
if chkvia(69.6,135.45,"CC2",verbose="CC2v1") and \
   check([(69.6,135.45),(70.8,135.9)], 0.125, B, "CC2", verbose="CC2-b1") and \
   check([(70.8,135.9),(76.9,135.9)], 0.125, B, "CC2", verbose="CC2-b2") and \
...
# ---- LED6_K:西绕 + LED 行上方 y79.05 走廊,北入 LED6.2 ----
pk = [(16.05,83.56),(16.05,83.3),(15.0,82.9),(13.55,82.3),(13.55,79.05),(17.31,79.05),(17.31,80.2)]
if check(pk[1:], 0.125, F, "LED6_K", verbose="LED6K", margin=0.18):
    trk(pk, "LED6_K", 0.25)
else:
    WARN.append("LED6_K")
