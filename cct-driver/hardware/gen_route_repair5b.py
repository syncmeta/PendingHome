...
# 1. V24_PROT 残段按几何删(B 层、经过 (15.76,94.58))
for t in live():
    if is_via(t) or t.GetNetname() != "V24_PROT" or t.GetLayer() != B:
        continue
    x1, y1, x2, y2 = ends(t)
    if min(abs(x1-15.76), abs(x2-15.76)) < 0.2 and max(y1, y2) > 90:
        kill(t)
        print(f"删残段 ({x1:.2f},{y1:.2f})->({x2:.2f},{y2:.2f})")

# 2. SW_NODE pad8 逃逸线西弯,给 pad9 GND 短接让位
del_exact("SW_NODE", [(30.1,103.72,30.1,105.4),(30.1,105.4,29.0,106.6)])
p_sw = [(30.1,103.72),(30.1,104.3),(29.2,105.5),(29.0,106.6)]
if check(p_sw[1:], 0.5, F, "SW_NODE", verbose="SWNODE"):
    trk(p_sw, "SW_NODE", 1.0)
else:
    WARN.append("SW_NODE")
    trk([(30.1,103.72),(30.1,105.4),(29.0,106.6)], "SW_NODE", 1.0)

# 3. SW_IN3 过孔南移(避 1mm 宽 V3P3 横线)
del_exact("SW_IN3", [(63.8,137.5,63.8,137.15),(63.8,136.95,64.55,135.35),(64.55,135.35,64.7,135.2)])
del_via_at([(63.8,136.95)])
trk([(63.8,137.5),(63.85,137.32)], "SW_IN3", 0.25)
via(63.85,137.3,"SW_IN3")
trk([(63.85,137.3),(64.55,135.5),(64.7,135.2)], "SW_IN3", 0.25, layer=B)

# 4. CH2_CW_GR 重复过孔:全删,挪位重打 + 双层短接
del_via_at([(40.6,56.9)], tol=0.2)
via(40.75,57.05,"CH2_CW_GR")
trk([(40.6,56.9),(40.75,57.05)], "CH2_CW_GR", 0.25)
trk([(40.6,56.9),(40.75,57.05)], "CH2_CW_GR", 0.25, layer=B)
