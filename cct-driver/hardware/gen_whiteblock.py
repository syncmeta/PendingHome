...
由 gen_silk.py 生成,不在本脚本的清理窗口内(它右沿 24.14 > CLEAN 右界 15)。

2026-08-13:整板绕板心转了 180°(见 gen_rotate180.py),板文件的方向此后
就是上墙安装的方向。本脚本的坐标已同步:白油块中心 101.8,91.8 → 8.2,53.2,
清理窗口 (95,85,108,99) → (2,46,15,60)。重跑会原地重建同一块白油,
位置一模一样。
...
CX, CY = 8.2, 53.2          # 块中心 = 原二维码油墨区(3.2~13.2 / 48.2~58.2)中心
CLEAR = 0.5                 # 白油块四周要求的最小净空 mm
EDGE_MIN = 3.0              # 白油块离板边最小距离 mm
TEXT = "cct-driver.local"   # 已废弃的明码文字,只用于清理(不再生成)
CLEAN = (2.0, 46.0, 15.0, 60.0)     # 清理窗口 x1,y1,x2,y2(覆盖旧 QR 含静区)
