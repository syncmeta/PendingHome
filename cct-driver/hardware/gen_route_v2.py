...
# ⚠️ U1 身上有**两只** V24_BUS 脚:pad8(IN−,分流器下游侧)和 pad9(VBUS,母线电压
# 检测,8/16 那一轮才补进原理图的),脚间距 0.5mm。上面那句 `P("U1","V24_BUS")` 只拿得到
# 其中一只 —— PADXY 每个 (位号, 网络) 只存一只脚,`chain()` 用的也是同一张表,
# 所以另一只脚**谁都看不见**,就那么悬着。这才是「V24_BUS 断路」的真身:
# 不是被 V24_PROT 占掉了那 2.7mm 走廊(那一段其实早就走通了),而是这 0.5mm 没人连。
# 线宽只能取 0.2mm:往东 0.5mm 就是 pad10(V24_PROT),0.3mm 宽的线端帽过去只剩
# 0.20mm,正好卡在 TRUNK 类的间距线上;0.2mm 宽留 0.25mm,有余量。
# 这两只脚都是高阻输入,不过电流,细线无所谓。
_u1_in, _u1_vbus = PADALL[("U1", "8")], PADALL[("U1", "9")]
hpath([_u1_in, _u1_vbus], F, 0.2, "V24_BUS", "U1 pad8(IN−) ↔ pad9(VBUS)")
...
for k_, (netname, src_ref) in enumerate(A3_BUS):
    # 车道带上沿卡在 U4 下排焊盘的下沿(22.65)之外,下沿卡在 A1 那排小件的上沿(30.12)
    # 以内。间距 0.68 不是拍的:每根车道在源头那一端都有一颗过孔,而更靠上的长车道
    # 会从这颗过孔头顶横过去,所以间距下限 = 过孔半径 0.35 + 间距 0.205 + 线半宽 0.125
    # = 0.68。10 根摊在 23.00–29.12。
    #
    # ⚠️ 上沿 23.25→23.00、间距 0.70→0.68,一共把最下那根车道抬高 0.43mm。
    # **这 0.43mm 是给 A1 那排去耦/上拉件让出的唯一一条逃生道**(见文末 V3P3 横轨):
    # 原先最下那根车道在 29.55,离 C10/C12 的焊盘上沿(30.12)只剩 0.57mm,
    # 中间连一根 0.25mm 的线都塞不下 —— 那排件于是三面封死,
    # C11 两只脚、R4 的 3V3 脚、C12 的地脚**实测全是浮的**(顶层碎铜里没有任何过孔)。
    # 抬高之后这条缝变成 1.0mm,正好够走一根 0.25mm 的轨。上沿 23.00 对 U4 焊盘
    # 下沿 22.65 留 0.35mm > 0.2mm 的规则值。
    lane = 23.0 + 0.68 * k_
...
# ------------------------------------------------------- A1 那排去耦/上拉件
# C10 / C11 / R4 / C12 / R5 一排五颗压在 y=30.8:上面是十条横向车道带、
# 下面 0.4mm 是 EN 去按键那一横、底层整片被 12 路 PWM 竖着占满 —— 三面封死。
#
