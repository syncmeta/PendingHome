...
# ---- 冗余 V24_PROT 粗馈线删除(PROT_RISER_B 覆铜已承担该路径) ----
del_exact("V24_PROT", [(8.64,70.17,16.28,77.81),(8.64,57.31,8.64,70.17),
                       (16.28,77.81,15.76,78.33),(15.76,78.33,15.76,94.58)])
del_via_at([(16.28,77.81)])

# ---- USB_VBUS ----
p1 = [(44.36,127.1),(52.0,130.6),(73.3,130.6)]
p2 = [(73.3,130.6),(73.6,131.5),(73.6,137.5),(73.6,140.85)]
p3 = [(73.6,140.85),(78.4,140.85)]
p4 = [(78.4,140.85),(78.4,138.4)]
okv = check(p1, 0.25, B, "USB_VBUS", verbose="VBUS-1") and \
      chkvia(73.3,130.6,"USB_VBUS",verbose="VBUS",margin=0.2) and \
      check(p2, 0.15, F, "USB_VBUS", verbose="VBUS-2", margin=0.13) and \
      ...
      check(p4, 0.15, F, "USB_VBUS", verbose="VBUS-4", margin=0.13)
if okv:
...
# ---- CC2:R10 竖放,B5 从 A8/B4A9 缝正中 0.2mm 进入 ----
for fp in board.GetFootprints():
    if fp.GetReference() == "R10":
        fp.SetPosition(VECTOR2I(FromMM(69.6), FromMM(136.5)))
        fp.SetOrientationDegrees(90)
p_e1 = [(76.9,135.9),(77.725,137.4)]
p_e2 = [(77.725,137.4),(77.725,138.4)]
if chkvia(69.6,135.45,"CC2",verbose="CC2v1") and \
   check([(69.6,135.45),(70.8,135.9),(76.9,135.9)], 0.125, B, "CC2", verbose="CC2-b") and \
   chkvia(76.9,135.9,"CC2",verbose="CC2v2") and \
   check(p_e1, 0.125, F, "CC2", verbose="CC2-e1", margin=0.15) and \
   check(p_e2, 0.10, F, "CC2", verbose="CC2-e2", margin=0.11):
    trk([(69.6,135.75),(69.6,135.45)], "CC2", 0.25)
    via(69.6,135.45,"CC2")
    trk([(69.6,135.45),(70.8,135.9),(76.9,135.9)], "CC2", 0.25, layer=B)
    via(76.9,135.9,"CC2")
    trk(p_e1, "CC2", 0.25)
    trk(p_e2, "CC2", 0.2)
else:
...
# ---- LED6_K:B 跳,终点 LED6.2 盘中孔 ----
if chkvia(16.05,82.7,"LED6_K",verbose="LK1") and chkvia(17.5,81.2,"LED6_K",verbose="LK2") and \
   check([(16.05,82.7),(17.5,81.2)], 0.15, B, "LED6_K", verbose="LK-B"):
    trk([(16.05,83.56),(16.05,83.1),(16.05,82.7)], "LED6_K", 0.3)
    via(16.05,82.7,"LED6_K")
    trk([(16.05,82.7),(17.5,81.2)], "LED6_K", 0.3, layer=B)
    via(17.5,81.2,"LED6_K")
else:
    WARN.append("LED6_K")
