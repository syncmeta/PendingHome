...
# 喂到**同一块金属**上。
#
# ⚠️ F1 就是后者,而且是实打实的短路:Keystone 3557-2 是「一颗保险丝、两个夹子」的座,
#    每个夹子两根引脚(焊盘 1+2 是同一个夹子,3+4 是另一个)。老板文件把 `V24_IN` 喂给
#    焊盘 1、`V24_FUSED` 喂给焊盘 2 —— 两者被夹子的金属短接,**15A 主保险丝被完全旁路**。
#    这里改成 引脚1 → 焊盘 1+2、引脚2 → 焊盘 3+4。
#
# 表以外的封装一律「引脚号 = 焊盘号」。verify_netlist_v2.py 会把这张表打印出来备查。
PAD_GROUPS = {
    "F1": {"1": ["1", "2"], "2": ["3", "4"]},
}
...
TPS = [
    ...
    ("TP7", "MASTER_OFF_TP", 26.0,  58.5),    # A5:总断路控制焊盘
...
at("LED1", 30.0,  8.0, 0)
at("R8",   34.5,  8.0, 0)
at("C10",  10.5, 26.0, 0)       # V3P3 去耦,贴 U4 pin2
at("C11",  15.0, 26.0, 0)
at("R4",   19.5, 26.0, 0)       # V3P3–EN
at("C12",  24.0, 26.0, 0)       # EN–GND
at("R5",   28.5, 26.0, 0)       # V3P3–IO0
...
at("C35",  73.0, 31.0, 0)       # V24_LOGIC 体电容(包络 12.04 × 8.60)
at("C32",  84.0, 28.5, 180)     # VIN 陶瓷,1 脚朝 U2
at("C33",  89.5, 28.5, 180)
at("C34",  94.0, 27.5, 180)
at("U2",   88.0, 36.0, 180)     # TPS54360:VIN/EN/RT/BOOT 在上排,SW/FB/COMP/GND 在下排
at("L1",  101.0, 36.0, 0)       # 紧接 SW(U2 8 脚到 L1 1 脚约 10mm)
at("D2",   86.0, 45.0, 0)       # 续流,贴 SW/GND
at("C38",  92.5, 31.0, 0)       # Cboot,贴 U2 的 1 脚 3.4mm
at("R62",  84.0, 31.0, 0)       # RT/CLK,贴 U2 的 4 脚 3.0mm
...
# ---------------- A5 栅极驱动区 x 4–101 y 54–63 ----------------
at("U7",   20.0, 58.5, 180)     # 驱 CH5/CH6 —— 压在它们的 x 质心上
at("C15",  15.0, 58.5, 0)
at("U6",   68.0, 58.5, 180)     # 驱 CH1–CH4
at("C14",  63.0, 58.5, 0)
at("R2",   30.0, 58.5, 0)       # 总断路:TP7 → R2 → Q3 基极
at("Q3",   34.0, 58.5, 0)
at("R3",   38.0, 58.5, 0)
at("R13",  44.0, 58.5, 0)       # /OE 失效安全:R13 上拉
at("Q6",   48.0, 58.5, 0)
at("R14",  52.0, 58.5, 0)
at("R15",  56.0, 58.5, 0)
