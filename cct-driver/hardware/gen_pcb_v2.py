...
HOLES = [
    ("H1",   4.0,  36.0), ("H2", 126.0,  12.0), ("H3",  61.0,   5.0),
...
at("LED1", 28.5,  6.0, 0)   # 贴 U4 右侧的 LED_STATUS 脚(25.6, 6.88)
at("R8",   28.5, 10.0, 0)
at("C10",  10.5, 30.8, 0)       # V3P3 去耦。这一排整体下移 4.8mm,
                                # 为的是在 U4 下方腾出 y 23.25–29.55 那条横向车道带 ——
                                # 那条带要塞 10 根横穿信号,**间距下限是量出来的**:
                                # 过孔半径 0.35 + 间距 0.205 + 线半宽 0.125 = 0.68,取 0.70。
                                # 10 根 = 9 × 0.70 = 6.30mm,上下各留 0.55 给 U4 焊盘和这一排。
at("C11",  15.0, 30.8, 0)
at("R4",   19.5, 30.8, 0)       # V3P3–EN
at("C12",  24.0, 30.8, 0)       # EN–GND
at("R5",   28.5, 30.8, 0)       # V3P3–IO0
at("SW2",  11.0, 35.0, 0)       # EN(复位);x 让开 H1(4,36) 的 courtyard
at("SW1",  22.0, 35.0, 0)       # IO0(BOOT)

# ---------------- A2 USB / 串口区 x 44–66 y 0–62 ----------------
at("J2",   52.0,  6.5, 180)     # Type-C 上板边,插拔朝外
# CC 下拉挪到 J2 两侧的**外面**去 —— J2 那一排触点是 0.5mm 间距,而 Type-C 是
# 可翻转的:D+ / D− / VBUS 各有 A、B 两个触点,必须在板上并起来。并线要换层,
# 换层要过孔,过孔要 1.01mm 的地方(0.6 + 两侧 0.205)。原来 R9/R10 就压在
# 触点正下方那 4.6mm 里,把唯一能放过孔的窗口占死了。
at("R10",  45.0, 11.5, 180)     # CC2 下拉(pad1 在右,朝 J2)
at("R9",   59.0, 13.0, 0)       # CC1 下拉
at("C13",  45.0, 18.0, 0)
at("U5",   52.0, 18.0, 0)       # CH340C
at("Q4",   46.0, 46.5, 0)   # 自动下载那四件排成一行,压在 PWM 车道带以下、5V 横轨以上
at("R11",  50.0, 46.5, 0)
at("Q5",   54.0, 46.5, 0)
at("R12",  58.0, 46.5, 0)

# ---------------- A3 对外接口 x 29–130 y 0–26 ----------------
# ⚠️ 2026-08-17 重排:**干接点与 UART2 从右上角搬到 MCU 这一侧**。
# 原来 J9/J10/J11 三个接口全挤在右上角(x 66–130),而它们伺候的 MCU 在左上角 ——
# 8 根信号要横穿约 90mm,穿的正是全板最挤的那一块。这违反楼层规划自己的**原则 3**
#(器件压在它服务的对象身上:驱动器压在它驱动的那几列上,连接器同样该靠近它通信的那几个脚)。
#
# 搬完之后横穿那条带的信号从 8 根降到 2 根(只剩 I2C):
#   · 干接点 4 路:J11 与它的 12 颗串阻/上拉/消抖整块搬到 U4 右边,**全程本地**
#   · UART2 2 路:J10 从 x=94 挪到 x=74,单根长度 70mm → 45mm
#   · I2C 2 路:**J9 留在右上角不动** —— 它除了 MCU 还要往下喂 D0 里的 U1(INA237),
#     留在右边那一路才短。这是有意的不对称。
at("J11",  37.0,  5.0, 0)       # 干接点 5P
for i, x in enumerate((31.5, 35.0, 38.5, 42.0)):
    at(f"R{54+i}", x, 12.0, 0)  # 串阻(端子侧)
    at(f"R{58+i}", x, 15.5, 0)  # 上拉(MCU 侧)
    at(f"C{28+i}", x, 19.0, 0)  # 消抖
at("J10",  74.0,  5.0, 0)       # UART 4P
at("J9",  112.0,  5.0, 0)       # I2C Qwiic(留在右上角:它还要往下喂 U1)
...
# FB / COMP / EN 那三组分压排成 2 列 3 行,**不要全叠在同一个 x 上** ——
# 全叠一列的话,每颗的两只脚正上方正下方都是另一颗的脚,想从中间任何一只脚
# 出线都出不去(布线时逐条栽在这上面,报的挡路者永远是隔壁那只脚)。
# FB / COMP / EN 那三组分压排成 2 列,**不叠在同一个 x 上**(全叠一列的话,
# 每颗的两只脚上下都是另一颗的脚,从中间任何一只脚都出不去线);
# 而且整体让开 **x 64–72** —— 那是 12 路 PWM 最后扎进 U6 输入脚的那一束竖线。
at("R63",  75.0, 40.0, 0)       # FB 上
at("R64",  79.0, 40.0, 0)       # FB 下
at("R65",  75.0, 44.0, 0)       # COMP
at("C39",  79.0, 44.0, 0)
at("C40",  75.0, 48.0, 0)
at("R66",  79.0, 48.0, 0)       # EN 分压上
at("R67",  75.0, 52.0, 0)       # EN 分压下
...
at("C6",   117.0,  63.98, 0)
# ⚠️ C46 竖着骑在脊椎北沿上,不是随便摆的:它原来整个人躺在脊椎当中(102.6, 71.21),
# 于是**两只脚都在 24V 铜里** —— V24_BUS 那只脚没问题,GND 那只脚就彻底出不去了。
# 实测:它的地只剩顶层 4.36mm² + 底层 0.19mm² 两小片挖空区,和主地平面在两层上
# 都没有任何重叠,一颗过孔并不进去;0.05mm 细网格洪水填充也走不出脊椎
# (脊椎顶层是一整片不断的铜,底层在 x>95 那一段也是整片)。
# 要接通只有两条路:① 拿一根 GND 线横切脊椎 —— 那是给 12A 母线开一道口子,不行;
# ② 把它挪到脊椎边上,让 V24_BUS 那只脚留在脊椎铜里、GND 那只脚落到脊椎外的地上。
# 选 ②:转 90° 竖过来,骑在脊椎**北**沿(y=65)上 —— 下脚(pad1,V24_BUS)y≈66.5
# 还在脊椎铜里,直接贴上去;上脚(pad2,GND)y≈63.5 落在脊椎北边那片地里,回路 <3mm,
# 比原来「紧挨 C4/C5 的地端」还短。
# 为什么是北沿不是南沿:南沿那一段(x 101–103)被 C5 的 4.6mm 长焊盘占着,
# 再往西就出了 D0 分区、掉进 CH1 通道列里。北沿 x 99–107 / y 58–65 实测是空的,
# 只有几颗地缝合过孔。
# x=106.3 是量出来的,而且**只有 0.16mm 的活动余地**,两头都顶死:
#   · 西:脊椎的缝合过孔每 3mm 一列(…101.5 / 104.5),2.7mm 宽的焊盘**塞不进
#     两列之间那 2.4mm**,只能放到最东那一列的东边。过孔外沿 104.8,
#     焊盘左沿 = x − 1.35 要躲开它 → x ≥ 106.15。(两者同网 V24_BUS,
#     压上去不算间距违规,但那就成了 via-in-pad,不干净。)
#   · 东:U1 的 I2C 脚在 108.10。下面那条摆位自检按「真实焊盘包络 + 0.45mm」判,
#     实测 x = 106.45 就报 0.14mm 相交 → x ≤ 106.31。
# 取 106.3。再往东一点点自检就红,再往西一点点就压在缝合过孔上。
at("C46",  106.30, 65.00, 90)   # V24_BUS 高频陶瓷(1210,4.68 宽)
...
ZONES = {
    "A0": ("天线净空区",        (0, 8),     (0, 25)),
    "A1": ("主控区",            (0, 30),    (0, 62)),
    "A2": ("USB / 串口区",      (44, 66),   (0, 62)),
    "A3": ("传感器 / 干接点区", (29, 130),  (0, 26)),
...
DEVIATIONS = {
    ...
    "C46": "新增件,V24_BUS 高频陶瓷。floorplan §C3 建议放脊椎最远端(C6),"
           ...
           "一度整个人躺在脊椎当中(102.6,71.21),结果 GND 那只脚被 24V 铜四面围死、"
           "两层都出不去(check-floating-pads.py 量出来的);现在转 90° 骑在脊椎北沿上,"
           "V24_BUS 那只脚留在脊椎铜里、GND 那只脚落进北边那片地,回路 <3mm",
    "R68": "新增件,进线阻尼电阻 → D0",
}
