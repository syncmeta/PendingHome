...
> **实际未做(2026-08-11 核实)**:板文件里没有任何独立的 F.Mask / F.Paste 图形,主脊椎的加锡开窗区并未实现,目前是纯 1oz 铜 + 过孔缝合。不补做 —— 载流以 12A 连续 / 15A 短时热像测试验收,不够再走 M3 孔位外接铜排。改用 OSP 后更要注意:**OSP 不会给铜面增厚**。详见规格书 §7。
...
- [ ] 表面处理:**OSP**(下单页免费;沉金实际加价 100 多元,原写的"约 ¥8"是按 140元/㎡ 线性外推估错了 —— 见规格书 §8.4b 末)。**别选喷锡**(不平整,USB-C 0.5mm 间距焊盘吃不消)
- [ ] 阻焊:**哑黑**(白丝印);哑黑需在文件里预留嘉立创订单二维码的 8×8mm 白油块
- [ ] 数量 5 片
